...
template<class Type>
class unorderedLinkedListType: public linkedListType<Type> {
public:
	...
	void deleteAllOccurrencesOfInfo(const Type& deleteItem);
...
template<class Type>
bool unorderedLinkedListType<Type>::search(const Type& searchItem) const {
...
template<class Type>
void unorderedLinkedListType<Type>::deleteLeastInfo() {
	nodeType<Type> *current; //pointer to current node in iteration
	nodeType<Type> *least; //current least info
	current = this->first;
	least = this->first;

	//check and compare each node's info with the smallest info
	while (current != nullptr) {
		if (current->info < least->info) {
			least = current;
		}
		current = current->link;
	}

	//delete the first occurrence of the least info
	deleteNode(least->info);
}

template<class Type>
void unorderedLinkedListType<Type>::deleteAllOccurrencesOfInfo(const Type& deleteItem) {
	nodeType<Type> *current; //pointer to traverse the list
	nodeType<Type> *trailCurrent; //tracks the previous node
	current = this->first;
	...
	if (current == nullptr) {
		...
		if (this->first->info == deleteItem) {
			//the search info is the first item in the list
			current = this->first;
		...
		while (current != nullptr) {
			if (current->info != deleteItem) {
				...
				//not the current node in the iteration
				...
				if (current != this->last) {
					...
					//it's not the last item. delete it and update
...
template<class Type>
void unorderedLinkedListType<Type>::insertFirst(const Type& newItem) {
...
template<class Type>
void unorderedLinkedListType<Type>::insertLast(const Type& newItem) {
	nodeType<Type> *newNode; //pointer to create the new node
...
template<class Type>
void unorderedLinkedListType<Type>::deleteNode(const Type& deleteItem) {
	nodeType<Type> *current; //pointer to traverse the list
